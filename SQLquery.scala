...
object SQLquery {
   ...
   {     
      ...
      val groupedRDD = sortedIntListRDD.groupBy(x=>x(1))
      val filteredListRDD = groupedRDD.map(_._2.filter(x => x(2)>=lower_bound && x(2)<=upper_bound)).map(_.toList).filter(_.size>0)
  ...
  def main(args: Array[String]) {
    ...
    //val Results = getResults(sc, 60, 120, "/home/bigdata/Assignment2/Data/OrderDetails")
